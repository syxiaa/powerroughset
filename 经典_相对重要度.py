    ...
    if len(data) != 0:
        ...
        while redund_atts:
            ...
            for index in redund_atts:   #计算出每个属性的属性重要度，选出属性重要度最高的属性
                ...
                num_class = len(new_eql_class) - len(current_equ_class)  # 新增等价类的个数
                ...
                num=num_re_111-num_re_222
                if num_class != 0:  #计算属性相对重要度
                    if num/ num_class>max_num:
                        max_num=num/ num_class
                        num_re=num_re_111
                        prepare_att = index
            if prepare_att < 0:
                break
...
for key in keys:
    ...
    for c in range(len(classifiers)):
        ...
        if re != [i for i in range(len(data[0, :-1]))]:
            ...
            with open(r'E:\\shiyanjieguo\\10.24\\经典_相对正域.csv', 'a+', encoding='utf-8-sig',
                      newline='') as f:
                writer = csv.writer(f)
